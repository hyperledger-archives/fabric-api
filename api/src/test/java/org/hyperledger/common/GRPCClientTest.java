...
public class GRPCClientTest {
    ...
    @Before
    public void setUp() {
        grpc = new GRPCClient("localhost", 30303, 31315);
    }

    @Test
    public void getNonExistingTransaction() throws BCSAPIException {
        APITransaction res = grpc.getTransaction(TID.INVALID);
        assertTrue(res == null);
    }
    ...
    public void sendTransaction() throws HyperLedgerException, BCSAPIException, InterruptedException {
        ...
        grpc.sendTransaction(tx);

        Thread.sleep(1500);

        APITransaction res = grpc.getTransaction(tx.getID());
        assertEquals(tx, res);
    }
}
